...
class WorkloadAndAcceleratorParserStage(Stage):
    ...
    def run(self):
        workload, workload_dict = parse_workload_from_path(self.workload_path)
        accelerator = parse_accelerator_from_path(self.accelerator_path)
        sub_stage = self.list_of_callables[0](self.list_of_callables[1:], accelerator=accelerator, workload=workload, workload_dict=workload_dict, **self.kwargs)
        for cme, extra_info in sub_stage.run():
            yield cme, extra_info
