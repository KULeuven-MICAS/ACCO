...
class DFInfiniteLoops(Exception):
    pass

class DFMultiOutputVilation(Exception):
    pass
...
def get_largest_alive_size(corrected_workload: DNNWorkload, permanent_data: Dict[LayerNode, int] = {}):
    ...
    for n in nx.topological_sort(corrected_workload):
        ...
        si = sum(
            n.operand_size_elem[I] * n.operand_precision[I] for I in n.input_operands if I not in n.constant_operands)
...
def get_effective_output_to_cache(to_cache, node, make_caching):
    ...
    :return:
    """
    assert (node not in to_cache or not to_cache[node] or all(
        to_cache[node][i][0] == to_cache[node][next(iter(to_cache[node]))][0] for i in to_cache[node]))
...
def backpropagate_tilesize(workload: DNNWorkload, tile_x, tile_y,
    ...
    # firstly align the B-dim size
    on = list(workload.topological_sort())[-1]
    B_size = on.loop_dim_size['B']
    for layer in workload.topological_sort():
        layer.loop_dim_size['B'] = B_size
    ...
    for on_i in on_i_list:
        ...
        while len(nodes_to_trace_over) > 0:
            ...
            for I, input_node in n.input_operand_source.items():
                ...
                # TODO: an index-based calibre to cancel padded-0 -> pass info and compute at cme?
    ...
    # print(usefull_nodes_output_sizes)
    return list(usefull_nodes_output_sizes.keys()), \
        {k: get_effective_output_to_cache(columns_to_cache_output, k, make_horizontal_caching)[:-1] for k in
         columns_to_cache_output}, \
        columns_to_cache_input, \
        {k: get_effective_output_to_cache(rows_to_cache_output, k, make_vertical_caching)[:-1] for k in
         rows_to_cache_output}, \
        rows_to_cache_input

def causal_to_tcn_transform(original_workload: DNNWorkload,
                            workload_lut: dict,
                            dry_run: bool = False,
                            layer_output_dim: dict = {},
                            initial_dilation_factor: int = 1,
                            global_dilation_out_factor: dict = {},
                            df_tcn_batch_amount: int = 1)\
        -> Tuple[Dict[str, Dict[str, int]], Dict[str, int], Dict[str, int]]:
    # print(list(usefull_workload.topological_sort()))
    sy_ref = 1  # assume the sequential raw input is generating on x-dim (sx=1, sy=0)
    dy_factor = initial_dilation_factor
    layer_dilation_in_dict = {}
    layer_dilation_out_dict = {}

    if df_tcn_batch_amount == -1:
        assert(global_dilation_out_factor), "global_dilation_out_factor is necessary if the batch of substack is not explicitly defined."

    usefull_workload = pickle_deepcopy(original_workload) if dry_run else original_workload

    for layer_node in usefull_workload.topological_sort():
        # print(i, layer_node, layer_node.id)

        # replace node with new Layer_Node():
        # use the workload_dict reference to construct new Layer_Node and replace
        if isinstance(layer_node, InputLayerNode):
            continue  # skip input nodes
        elif layer_node.equation_relations:  # skip non-transformable nodes e.g. residual-addition
            assert (layer_node.equation_relations == workload_lut[layer_node.id][
                'equation_relations']), f"equation_relations not match: {layer_node.equation_relations} <-> {workload_lut[layer_node.id]['equation_relations']}"

            layer_dilation_in_dict[layer_node.__str__()] = int(dy_factor)

            # TODO: now only compatible with: basic conv2d+pool2d; residual;
            # sx_old = layer_node.pr_scaling_factors['IX']['ox']
            # dx_old = layer_node.pr_scaling_factors['IX']['fx']
            sy_old = layer_node.pr_scaling_factors['IY']['oy']
            dy_old = layer_node.pr_scaling_factors['IY']['fy']

            dy_new = dy_old * dy_factor
            if sy_old == sy_ref:
                sy_new = sy_old
            else:
                dy_factor *= sy_old / sy_ref
                sy_new = sy_ref

            layer_dilation_out_dict[layer_node.__str__()] = int(dy_factor)

            # equation_relations_new = [f'ix={sx_old}*ox+{dx_old}*fx', f'iy={sy_new}*oy+{int(dy_new)}*fy']
            equation_relations_new = []
            for equation_str in layer_node.equation_relations:
                if 'oy' in equation_str and 'fy' in equation_str:
                    equation_relations_new.append(equation_str.replace(f'{sy_old}*oy', f'{sy_new}*oy').replace(f'{dy_old}*fy', f'{int(dy_new)}*fy'))
                else:
                    equation_relations_new.append(equation_str)
            # print(f'equation_relations_new: {equation_relations_new}')
            # print(f'equation_relations_old: {layer_node.equation_relations}')

            layer_workload_dict = pickle_deepcopy(workload_lut[layer_node.id])
            layer_workload_dict['equation_relations'] = equation_relations_new
            # TODO: there will be more tiles here if padding is taken into account

            new_layer: LayerNode = LayerNode(layer_node.id, layer_workload_dict)
            new_layer.input_operand_source = layer_node.input_operand_source
            for p in usefull_workload.predecessors(layer_node):
                usefull_workload.add_edge(p, new_layer)
            for s in usefull_workload.successors(layer_node):
                usefull_workload.add_edge(new_layer, s)
            usefull_workload.remove_node(layer_node)
        else: # residual addition layer
            layer_dilation_in_dict[layer_node.__str__()] = int(dy_factor)
            layer_dilation_out_dict[layer_node.__str__()] = int(dy_factor)

    # input_operand_source rebuild to link with new nodes
    for layer_node in usefull_workload.topological_sort():
        if usefull_workload.in_degree(layer_node) != 0:
            # print(layer_node.input_operand_source)
            reversed_input_operand_source: dict = {v.__str__(): k for k, v in layer_node.input_operand_source.items()}
            for p in usefull_workload.predecessors(layer_node):
                # assume the network structure is preserved and node-name keeps the same
                _k = reversed_input_operand_source[p.__str__()]
                layer_node.input_operand_source[_k] = p
                # print(f'connected node {p.__str__()} -> {layer_node.__str__()}:input_source[{_k}]')

        if usefull_workload.out_degree[layer_node] == 0:
            if dry_run:
                # equivalent to the "leftover" mechanism in tiling, otherwise some
                # then we can specify a large-enough value to take 100% tile
                layer_node.loop_dim_size['OX'] = min(layer_output_dim['dry_run']['OX'], layer_output_dim[layer_node.__str__()]['OX'])
                layer_node.loop_dim_size['OY'] = min(layer_output_dim['dry_run']['OY'],
                                                     int(layer_output_dim[layer_node.__str__()]['OY'] * dy_factor))
                if df_tcn_batch_amount != -1:
                    layer_node.loop_dim_size['B'] = df_tcn_batch_amount
                else:
                    layer_node.loop_dim_size['B'] = math.ceil(global_dilation_out_factor[layer_node.__str__()] / dy_factor)
            elif layer_output_dim:
                # Update the updated dims, but should keep other dimensions same as the original
                layer_node.loop_dim_size['OX'] = layer_output_dim[layer_node.__str__()]['OX']
                layer_node.loop_dim_size['OY'] = int(layer_output_dim[layer_node.__str__()]['OY'] * dy_factor)  # initial*scaling
                # layer_node.loop_dim_size['B'] = math.ceil(df_tcn_frame_amount / dy_factor)
                if df_tcn_batch_amount != -1:
                    layer_node.loop_dim_size['B'] = df_tcn_batch_amount
                else:
                    layer_node.loop_dim_size['B'] = math.ceil(global_dilation_out_factor[layer_node.__str__()] / dy_factor)
            else:
                # keep the original workload definition, used by the initial global_dilation inference
                pass


    # compute full feature map sizes first, again
    backpropagate_tilesize(usefull_workload, None, None, False, False, False, False)

    # for layer_node in usefull_workload.topological_sort():
    #     assert (layer_node.loop_dim_size['B'] == 1), "Temporary Batch Dim Lock to 1"
    if dry_run:
        _sum_o = sum(layer.operand_size_bit['O'] for layer in usefull_workload.topological_sort())
        _sum_i = sum(layer.operand_size_bit['O'] for layer in usefull_workload.topological_sort() if hasattr(layer.operand_size_bit, 'I'))
        return _sum_o
    else:
        return {l.__str__(): l.loop_dim_size for l in list(usefull_workload.topological_sort())}, \
            layer_dilation_in_dict, \
            layer_dilation_out_dict

def workload_substack_cut(original_workload: DNNWorkload, cut_before: int, cut_after: int):
    substack: DNNWorkload = pickle_deepcopy(original_workload)
    first_input = next(original_workload.topological_sort())
    ...
    if cut_before is not None:
        ...
        all_successors_of_final = [n.id for n in
                                   original_workload.successors(original_workload.get_node_with_id(cut_before))]

        for p in set(all_predecessors):
            substack.remove_node(p)

        for p in set(all_predecessors):
            for n in original_workload.successors(original_workload.get_node_with_id(p.id)):
                if n.id in list(nn.id for nn in substack.nodes()) and n.id not in all_successors_of_final:
                    all_successors_of_final.append(n.id)
    ...
    if all_successors_of_final:
        sil_counter = 0  # commented out to use unique-id instead
        input_nodes = set()
        for n in all_successors_of_final:
            input_nodes.update(original_workload.get_node_with_id(n).input_operand_source.values())
        ...
        for n in input_nodes:
            ...
            # unique substack new layer id
            # sil_counter = n.id
            new_input_node = InputLayerNode(f'substack_input_layer_{sil_counter}', input_loop_dim_sizes,
                                            input_precision, first_input.memory_operand_links,
                                            first_input.core_allocation)
...
class DepthFirstStage(Stage):
    ...
    def __init__(self, list_of_callables, *,
                 ...
                 df_stack_cuts: list,
                 df_tcn_transform: bool,
                 result_saving_path: str,
                 df_tcn_global_initial_dilation: int = 1,
                 df_tcn_batch_amount: int = 1,
                 df_tcn_substack_cut_max: int = 0,
                 already_computed_cut_ranges: list = [],
                 **params):
        """
        ...
        self.df_tcn_transform = df_tcn_transform
        self.result_saving_path = result_saving_path
        self.workload_dict = workload_dict

        self.df_tcn_global_initial_dilation = df_tcn_global_initial_dilation
        self.df_tcn_batch_amount = df_tcn_batch_amount # global override for all the substack's batch otherwise they are inferred
        self.df_tcn_substack_cut_max = df_tcn_substack_cut_max
        self.already_computed_cut_ranges = already_computed_cut_ranges
    ...
    def run(self):
        ...
        # LUT of the original entire workload dim_size
        backpropagate_tilesize(self.workload, None, None, False, False, False, False)
        df_original_output_dim_dict = {l.__str__(): l.loop_dim_size for l in list(self.workload.topological_sort())}
        for i in self.workload.topological_sort():
            if isinstance(i, LayerNode):
                print(i.__jsonrepr__())
                print(i.input_operand_source)
                print(self.workload.predecessors(i))
        print()

        # compute full feature map sizes first
            # LUT of the flattened entire workload dim_size
        if self.df_tcn_transform:
            _workload = pickle_deepcopy(self.workload)
            df_flattened_output_dim_dict, dilation_dict_in_total, dilation_dict_out_total = causal_to_tcn_transform(_workload, self.workload_dict)
            for layer_node in _workload.topological_sort():
                print(layer_node.operand_size_elem)
            del _workload

        print(df_original_output_dim_dict)
        # assert(0)


        # print(list(self.workload.topological_sort())[-1].__str__())
        # print(list(self.workload.topological_sort())[-1].operand_size_elem)
        # print(list(self.workload.topological_sort())[-2].__str__())
        # print(list(self.workload.topological_sort())[-2].operand_size_elem)
        # assert(0)

        # freedom dive for cut-tuple candidates
        layer_idx_list = list(i.id for i in self.workload)
        _MAX = max(layer_idx_list)
        for i in [-1, _MAX]:
            if i in layer_idx_list:
                layer_idx_list.remove(i)
            if i in self.df_stack_cuts:
                self.df_stack_cuts.remove(i)
        for idx in self.df_stack_cuts:
            layer_idx_list.remove(idx)
        _possible_cut_amount = int(sum((end-start)*(end-start+1)/2 for start, end in zip([-1] + sorted(self.df_stack_cuts), sorted(self.df_stack_cuts + [_MAX]))))
        print(_possible_cut_amount)
        print(self.df_stack_cuts)
        cut_range_candidate_list = []
        candidate_amount = 0
        additional_cuts = 0
        while True:
            if additional_cuts > self.df_tcn_substack_cut_max:
                break
            cut_id_candidate_list = []
            for i in range(additional_cuts + 1): # applies maximal X additional cuts
                cut_id_candidate_list += (list(itertools.combinations(layer_idx_list, i)))
            cut_id_candidate_list = list(sorted(list(i_tuple) + self.df_stack_cuts) for i_tuple in cut_id_candidate_list)
            for cut_candidate in cut_id_candidate_list:
                for cut_before, cut_after in zip([None] + cut_candidate, cut_candidate + [None]):
                    if not (cut_before, cut_after) in cut_range_candidate_list:
                        cut_range_candidate_list.append((cut_before, cut_after))
                        candidate_amount += 1
            if candidate_amount >= _possible_cut_amount:
                break
            additional_cuts += 1

        for cut_range in self.already_computed_cut_ranges:
            if cut_range in cut_range_candidate_list:
                cut_range_candidate_list.remove(cut_range)

        # check to remove invalid residual-connection cut ranges
        multi_output_layer_idx_list = list(node.id for node, out_degree in self.workload.out_degree() if out_degree > 1)
        print(f'multi-output nodes: {multi_output_layer_idx_list}')
        cut_range_candidate_list_residual_clear = []
        # a valid cut range must end with a multi-output node, or not contain any of them
        # TODO: this is not exhaustive, should upgrade to DiGraph based wild out-edges check
        for cut_before, cut_after in cut_range_candidate_list:
            _cut_before = cut_before if cut_before is not None else -1
            _cut_after = cut_after if cut_after is not None else _MAX
            ...
            try:
                for multi_output_node_idx in multi_output_layer_idx_list:
                    if multi_output_node_idx in cut_idx_list and _cut_after not in multi_output_layer_idx_list:
                        print(f'remove invalid cut range [{cut_before},{cut_after}] as it hits {multi_output_node_idx} and [{cut_after}] not in {multi_output_layer_idx_list}')
                        raise DFMultiOutputVilation
            except:
                pass
            else:
                cut_range_candidate_list_residual_clear.append((cut_before, cut_after))

        print(f'Already computed cut ranges #[{len(self.already_computed_cut_ranges)}]: {self.already_computed_cut_ranges}')
        print(f'Initial valid cut range #[{len(cut_range_candidate_list_residual_clear)}]: {cut_range_candidate_list_residual_clear}')
        first_input = next(self.workload.topological_sort())

        mh: MemoryHierarchy = self.accelerator.get_core(next(n for n in nx.topological_sort(self.workload) if hasattr(n,
                                                                                                                      'core_allocation')).core_allocation).memory_hierarchy
        on_chip_output_llc_cache_capacity_bit = mh.get_memory_levels('O')[-2].memory_instance.size
        on_chip_output_smaller_cache_capacity_bit = mh.get_memory_levels('O')[1].memory_instance.size
        ###################################
        ...
        cut_before: int
        cut_after: int
        cut_ranges_to_be_computed = []
        for cut_before, cut_after in cut_range_candidate_list_residual_clear:  # works with empty list of df_stack_cuts
            if cut_before == list(self.workload.topological_sort())[-1].id:
                continue

            substack = workload_substack_cut(self.workload, cut_before, cut_after)
            if self.df_tcn_transform:
                df_original_output_dim_dict.update({'dry_run': {'OX': self.tilesize_x, 'OY': self.tilesize_y}})

                for layer_node in substack.topological_sort():
                    if isinstance(layer_node, LayerNode):
                        initial_dilation_ref = dilation_dict_in_total[layer_node.__str__()]
                        break
                    else:
                        continue
                _initial_dilation_factor = initial_dilation_ref if self.df_tcn_global_initial_dilation == -1 else self.df_tcn_global_initial_dilation

                _sum_bits = causal_to_tcn_transform(substack,
                                                    self.workload_dict,
                                                    dry_run=True,
                                                    layer_output_dim=df_original_output_dim_dict,
                                                    initial_dilation_factor=_initial_dilation_factor,
                                                    global_dilation_out_factor=dilation_dict_out_total,
                                                    df_tcn_batch_amount=self.df_tcn_batch_amount)

                if _sum_bits >= on_chip_output_llc_cache_capacity_bit:
                    print(f'Note: [{cut_before}]->[{cut_after}] bits overflow L2: {_sum_bits} > {on_chip_output_llc_cache_capacity_bit}')
                elif _sum_bits >= on_chip_output_smaller_cache_capacity_bit:
                    print(f'Note: [{cut_before}]->[{cut_after}] bits overflow L1: {_sum_bits} > {on_chip_output_smaller_cache_capacity_bit}')
                else:
                    print(f'Note: [{cut_before}]->[{cut_after}] bits tiny enough: {_sum_bits} < {on_chip_output_smaller_cache_capacity_bit}')

                substacks.append([substack, cut_before, cut_after])
                cut_ranges_to_be_computed.append((cut_before, cut_after))

                # if _sum_bits >= on_chip_output_llc_cache_capacity_bit:
                #     print(f'cut [{cut_before}]->[{cut_after}] bits overflow: {_sum_bits} > {on_chip_output_llc_cache_capacity_bit}')
                #     del substack
                #     continue
                # else:
                #     print(_sum_bits)
        ...
        print(f'Final valid cut range #[{len(substacks)} ({len(cut_range_candidate_list_residual_clear)})]')
        ...
        for substack, cut_before, cut_after, input_ml, output_ml in substacks:
            ...
            cost_model_evaluations_per_layer_row_beginning = defaultdict(return_empty_list)
            cost_model_evaluations_per_layer_row_internal = defaultdict(return_empty_list)
            cost_model_evaluations_per_layer_row_fin = defaultdict(return_empty_list)

            usefull_workload = pickle_deepcopy(substack)

            # transform the layer definition
            # and replace the node in workload DiGraph
            if self.df_tcn_transform:
                for layer_node in usefull_workload.topological_sort():
                    if isinstance(layer_node, LayerNode):
                        initial_dilation_ref = dilation_dict_in_total[layer_node.__str__()]
                        ...
                        continue
                if self.df_tcn_global_initial_dilation > initial_dilation_ref:
                    print(f'skip this substack [{cut_before}]--[{cut_after}] as global_initial_dilation too large: {self.df_tcn_global_initial_dilation} > {initial_dilation_ref}')
                    continue
                else:
                    _initial_dilation_factor = initial_dilation_ref if self.df_tcn_global_initial_dilation == -1 else self.df_tcn_global_initial_dilation
                    df_flattened_substack_output_dim_dict, \
                        substack_dilation_dict_in, \
                        substack_dilation_dict_out = causal_to_tcn_transform(usefull_workload,
                                                                             self.workload_dict,
                                                                             False,
                                                                             df_original_output_dim_dict,
                                                                             _initial_dilation_factor,
                                                                             dilation_dict_out_total,
                                                                             self.df_tcn_batch_amount)

                print(df_flattened_substack_output_dim_dict)
                print(f'global reference of dilation-in:\n{dilation_dict_in_total}')
                print(f'substack dilation-in:\n{substack_dilation_dict_in}')
                print(f'substack dilation-out:\n{substack_dilation_dict_out}')
                for layer_node in usefull_workload.topological_sort():
                    if isinstance(layer_node, LayerNode):
                        print(layer_node.__jsonrepr__())

                # skip too large tile-size by comparing the last layer's OX, OY
                for layer in usefull_workload.topological_sort():
                    if isinstance(layer, LayerNode):
                        _dilation_in = dilation_dict_in_total[layer.__str__()]
                        break
                _last_layer_ = list(usefull_workload.topological_sort())[-1]
                _last_layer_dimension = _last_layer_.loop_dim_size
                _ox_total_stride = dilation_dict_out_total[_last_layer_.__str__()] / _dilation_in
                if self.tilesize_x >= 2 * _ox_total_stride * _last_layer_dimension['OX'] or self.tilesize_y >= 2 * _last_layer_dimension['OY']:
                    # print('Skip this tile:', self.tilesize_x, self.tilesize_y, _last_layer_dimension['OX'], _last_layer_dimension['OY'])
                    # yield None, (self.already_computed_cut_ranges + cut_ranges_to_be_computed, [], [])
                    continue
                else:
                    _evaluated += 1
            ...
            assert len(
                usefull_nodes) == 1, "DepthFirstStage does not support workloads (or substacks) with multiple outputs"
            ...
            def run_for_tilesize(tile_x, tile_y, weights_from_top,
                ...
                logger.debug("Running for " + str(tuple(
                    a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in
                    args)) + f"at position {tile_x0}, {tile_y0}")
                ...
                mh = self.accelerator.get_core(next(n for n in nx.topological_sort(usefull_workload_tile) if
                                                    hasattr(n, 'core_allocation')).core_allocation).memory_hierarchy

                memories_to_take_for_W = []
                if w_ml is not None:
                    for i, ml in enumerate(mh.get_memory_levels(next(
                            n for n in nx.topological_sort(usefull_workload_tile) if
                            hasattr(n, 'memory_operand_links') and 'W' in n.memory_operand_links).memory_operand_links[
                                                                    'W'])):
                ...
                for layer in usefull_workload_tile.topological_sort():
                    ...
                    while True:  # serves more as a goto target here
                        ...
                        assert (not is_substack_input or isinstance(layer,
                                                                    InputLayerNode)), "Found a layernode that has no incoming edge but is not an input. Check input_operand_source"
                        ...
                        if is_substack_input:  # until the one decided for this substack
                            ...
                            # TODO: patch for dilation-recompute, further verify if it is okay with all cases
                            # if self.df_tcn_transform:
                            #     _scaling_factors = layer.pr_scaling_factors['IY']
                            #
                            # if x0 != 0:
                            #     for I in layer.input_operands:
                            #         if I not in layer.constant_operands:
                            #             print(layer.pr_scaling_factors)
                            # else:
                            input_size = sum(
                                layer.operand_size_elem[I] * layer.operand_precision[I] for I in layer.input_operands if
                                I not in layer.constant_operands)
                        ...
                        if extra_input_memlevels_because_it_did_not_work:
                            memories_to_take_for_I.extend(mh.get_memory_levels(layer.memory_operand_links[next(
                                I for I in layer.input_operands if I not in layer.constant_operands)])[
                                                          len(memories_to_take_for_I):len(
                                                              memories_to_take_for_I) + extra_input_memlevels_because_it_did_not_work])
                        if extra_output_memlevels_because_it_did_not_work:
                            memories_to_take_for_O.extend(mh.get_memory_levels(layer.memory_operand_links['O'])[
                                                          len(memories_to_take_for_O):len(
                                                              memories_to_take_for_O) + extra_output_memlevels_because_it_did_not_work])
                        ...
                        if is_substack_input:
                            ...
                            to_copy.append(
                                (('SI', 'O'), input_ml, memories_to_take_for_O[-1], layer.operand_size_bit['O']))
                            logger.debug("Added to copy stack input -> input: " + str(tuple(
                                a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in to_copy[-1])))
                        else:
                            # output -> input
                            for I in layer.input_operand_source:
                                stored_as_output = memories_to_take_for_I_lookup_as_from_O[
                                    layer.input_operand_source[I].id]
                                if memories_to_take_for_I[-1] != stored_as_output[0] and not isinstance(
                                        layer.input_operand_source[I], InputLayerNode) \
                                        or stored_as_output[0] not in memories_to_take_for_I and isinstance(
                                    layer.input_operand_source[I], InputLayerNode):
                                    # the inputs where not stored where they need to be stored now -> copy them
                                    to_copy.append((('O', I), stored_as_output[0], memories_to_take_for_I[-1],
                                                    stored_as_output[1]))
                                    logger.debug("Added to copy output -> input: " + str(tuple(
                                        a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in
                                        to_copy[-1])))

                            # caches for horizontal resue -> input
                            if use_horizontal_cache_ml is not None and use_horizontal_cache_ml != \
                                    memories_to_take_for_I[-1]:
                                for I in layer.input_operands:
                                    if I not in layer.constant_operands:
                                        # copy cached inputs
                                        amount_bits_factors = {k: layer.loop_dim_size[k] for k in
                                                               layer.operand_loop_dim[I]['r']}
                                        for k in layer.operand_loop_dim[I]['pr']:
                                            amount_bits_factors[k] = layer.calc_tensor_dims(I, layer.loop_dim_size)[k]
                                        amount_bits_factors[horizontal_read_columns[(layer.id, I)][0]] = \
                                        horizontal_read_columns[(layer.id, I)][1]
                                        ...
                                        if amount_bits > 0:
                                            to_copy.append((('CI', I), use_horizontal_cache_ml,
                                                            memories_to_take_for_I[-1], amount_bits))
                                            logger.debug("Added to copy cached input (H) -> input: " + str(tuple(
                                                a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in
                                                to_copy[-1])) + " = " + '*'.join(
                                                str(f) for f in amount_bits_factors.values()))
                            ...
                            if use_vertical_cache_ml is not None and vertical_cache_ml != memories_to_take_for_I[-1]:
                                for I in layer.input_operands:
                                    if I not in layer.constant_operands:
                                        ...
                                        nb_columns = layer.loop_dim_size[
                                            column_dim] if column_dim in layer.loop_dim_size \
                                        ...
                                        amount_bits_factors[vertical_read_rows[(layer.id, I)][0]] = \
                                        vertical_read_rows[(layer.id, I)][1]
                                        amount_bits_factors['precision'] = layer.operand_precision['O_final']
                                        amount_bits = prod(amount_bits_factors.values())
                                        if amount_bits > 0:
                                            to_copy.append((
                                                           ('CI', I), use_vertical_cache_ml, memories_to_take_for_I[-1],
                                                           amount_bits))
                                            logger.debug("Added to copy cached input (V) -> input: " + str(tuple(
                                                a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in
                                                to_copy[-1])) + " = " + '*'.join(
                                                str(f) for f in amount_bits_factors.values()))

                        # give subsequent layers info on where to find there inputs = outputs of this layer
                        memories_to_take_for_I_lookup_as_from_O[layer.id] = memories_to_take_for_O[
                            -1], layer_output_size
                        ...
                        if not is_substack_input:
                            for I in layer.input_operands:
                                if I not in layer.constant_operands:
                                    while memories_to_take_for_I[-1] not in \
                                            memhier.get_operator_top_level(layer.memory_operand_links[I])[0]:
                        ...
                        if not is_substack_output:
                            while memories_to_take_for_O[-1] not in \
                                    memhier.get_operator_top_level(layer.memory_operand_links['O'])[0]:
                        ...
                        if [layer.memory_operand_links[I] for I in layer.input_operands] == ['I1', 'I1']:
                            ...
                            accelerator_mem_level_removed.get_core(
                                layer.core_allocation).recalculate_memory_hierarchy_information()
                            copied_I1 = True
                        else:
                            copied_I1 = False
                        ...
                        if not weights_from_top:
                            if layer.constant_operands:
                                if memories_to_take_for_W:
                                    while memories_to_take_for_W[-1] not in memhier.get_operator_top_level(
                                            layer.memory_operand_links[layer.constant_operands[0]])[0]:
                        ...
                        for tc in to_copy:
                            ...
                            source_op = 'O'
                            if tc[0][0] == 'CI':
                                source_op = layer.memory_operand_links[
                                    tc[0][1]]  # assume same operand as input (memorylevel is shared anyways)

                            source_memlevel = self.accelerator.get_core(
                                layer.core_allocation).memory_hierarchy.get_memorylevel_with_id(tc[1].get_id())
                        ...
                        dcl = DataCopyLayer(f'datamovement_b4_{layer}', to_copy_actions, self.accelerator,
                                            layer.core_allocation)  # will save to return this after the substage ran correctly

                        # done processing these, clear the array
                        to_copy.clear()

                        if extra_input_memlevels_because_it_did_not_work > 1e3:
                            raise DFInfiniteLoops()
                        ...
                        try:  # try except to catch edge cases where reserved space turns out not to be enough, in which case we throw stuff at the wall
                            if is_substack_input:
                                ...
                                logger.info(f"The spatial mapping dict applied is {cme.spatial_mapping.spatial_loop_dim_size}")

                                _exhausted = object()
                                if next(sub_stage_gen, _exhausted) is not _exhausted:
                                    raise ValueError(
                                        "Subcallable of df_pipeline yields more than one result, which df_pipeline does not know what to do with. Consider using a reduce.MinimalEnergyStage.")
                            ...
                            logger.info(
                                f"MemoryLevels were too optimistic => retrying for this layer with extra levels {extra_output_memlevels_because_it_did_not_work} for O and {extra_input_memlevels_because_it_did_not_work} for I")
                        ...
                        if make_vertical_cache_ml is not None and make_vertical_cache_ml != memories_to_take_for_O[-1] \
                            ...
                            if amount_bits > 0 and not isinstance(layer, InputLayerNode):
                                to_copy.append(
                                    (('O', 'CO'), memories_to_take_for_O[-1], make_vertical_cache_ml, amount_bits))
                                logger.debug("Added to copy output (V) -> cached_output: " + str(tuple(
                                    a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in
                                    to_copy[-1])) + " = " + '*'.join(str(f) for f in amount_bits_factors.values()))

                        # and now for horizontal reuse
                        if make_horizontal_cache_ml is not None and make_horizontal_cache_ml != memories_to_take_for_O[
                            -1] \
                                and layer.id in horizontal_write_columns:
                            amount_bits_factors = {k: layer.loop_dim_size[k] for k in layer.operand_loop_dim['O']['r']}
                            amount_bits_factors[horizontal_write_columns[layer.id][0]] = \
                            horizontal_write_columns[layer.id][1]
                            amount_bits_factors['precision'] = layer.operand_precision['O_final']
                            if subtract_vertical_from_horizontal is not None:
                                amount_bits_factors[subtract_vertical_from_horizontal[0]] -= \
                                subtract_vertical_from_horizontal[1]

                            amount_bits = prod(amount_bits_factors.values())
                            if amount_bits > 0 and not isinstance(layer, InputLayerNode):
                                to_copy.append(
                                    (('O', 'CO'), memories_to_take_for_O[-1], make_horizontal_cache_ml, amount_bits))
                                logger.debug("Added to copy output (H) -> cached_output: " + str(tuple(
                                    a if not isinstance(a, MemoryLevel) else a.memory_instance.name for a in
                                    to_copy[-1])) + " = " + '*'.join(str(f) for f in amount_bits_factors.values()))
                        break
            ...
            def determine_caching_for_tilesize(tile_x: int,
                ...
                tilesize_to_determine_cachesize = (
                tile_x, tile_y, first_x, first_y, horizontal_caching, vertical_caching)
                ...
                for n, I in to_cache_vertically_in:
                    ...
                    if dim in n.operand_loop_dim[I]['r']:
                        ...
                        h_dim_total_size = n.calc_tensor_dims(I, self.workload.get_node_with_id(n.id).loop_dim_size)[
                            h_dim]
                ...
                i_mem_op = next(n.memory_operand_links[I] for n in usefull_nodes for I in n.memory_operand_links if
                                I not in n.constant_operands and n.memory_operand_links[I] != 'O')
                ...
                if horizontal_caching:
                    ...
                    memory_hierarchy = self.accelerator.get_core(next(n for n in usefull_workload.topological_sort() if
                                                                      hasattr(n,
                                                                              'core_allocation')).core_allocation).memory_hierarchy
                    ...
                    if vertical_caching:
                        ...
                        for i, ml in enumerate(memory_hierarchy.get_memory_levels(o_mem_op)):
                            ...
                            if ml == horizontal_caching_ml:
                                size_to_fit -= sum(
                                    to_cache_overlapped_bits_in.values())  # only subtracting this if they are the same memorylevel, Otherwise we assume stored double
                ...
                # print(f'\tTile-({tile_x},{tile_y}), FirstXY-({first_x},{first_y}): peak_f_mem_size_h = {peak_f_mem_size_h}, peak_f_mem_size = {peak_f_mem_size}')
                ...
                weights_total = sum(
                    sum(layer.operand_size_bit[W] for W in layer.constant_operands) for layer in usefull_nodes)
                ...
                try:
                    ...
                    for i, ml in enumerate(mh.get_memory_levels(w_mem_op)):
                        ...
                        if ml == horizontal_caching_ml and ml == vertical_caching_ml:
                            size_to_fit = weights_total + peak_f_mem_size - sum(
                                to_cache_overlapped_bits_in.values())  # only subtracting this if they are the same memorylevel, Otherwise we assume stored double
                ...
                rows_to_read = {(l[0].id, l[1]): tuple(list(rows_to_read[l]) + [to_cache_horizontally_in[l][0]]) for l
                                in rows_to_read}
                ...
                determine_horizontal_caching_ml_for_tilesize_cache[tilesize_to_determine_cachesize] = \
                    (horizontal_caching_ml, vertical_caching_ml, columns_to_write, columns_to_read, rows_to_write,
                     rows_to_read, weight_ml)
            ...
            mh: MemoryHierarchy = self.accelerator.get_core(next(n for n in nx.topological_sort(self.workload) if
                                                                 hasattr(n,
                                                                         'core_allocation')).core_allocation).memory_hierarchy
            mh_sorted = list(nx.topological_sort(mh))
            # print(f'This is substack [{cut_before},{cut_after}]:')
            ...
            while y0 < total_OY:
                ...
                while x0 < total_OX:
                    ...
                    # print(f'\tWorking on Location-({x0},{y0}) with Tile-({this_tile_x},{this_tile_y}):')
                    ...
                    if self.horizontal_caching:
                        if x0 + self.tilesize_x >= total_OX:
                            horizontal_caching_info[(x0, y0)] = [None, defaultdict(return_emtpystr_0_list),
                                                                 horizontal_cache_read_columns]
                        elif x0 == 0:
                        ...
                        None, defaultdict(return_emtpystr_0_list), defaultdict(return_0))
                                    ...
                                    if ml == h:  # h hit first, so new one is higher
                                        ...
                                        for k in vertical_caching_info:
                                            if vertical_caching_info[k][0] is not None:
                                                vertical_caching_info[k] = (highest_vertical_cache_ml,) + \
                                                                           vertical_caching_info[k][1:]
                                        break

                        if y0 + self.tilesize_y >= total_OY:
                            vertical_caching_info[(x0, y0)] = (
                            None, defaultdict(return_emtpystr_0_list), vertical_read_rows)
                        else:
                            vertical_caching_info[(x0, y0)] = (
                            highest_vertical_cache_ml, vertical_write_rows, vertical_read_rows)
                    else:
                        vertical_caching_info[(x0, y0)] = (
                        None, defaultdict(return_emtpystr_0_list), defaultdict(return_0))
            ...
            try:
                ...
                while y0 < total_OY:
                    ...
                    cost_model_evaluations_per_layer_per_row = defaultdict(return_empty_list)
                    ...
                    while x0 < total_OX:
                        ...
                        for k, v in energy_of_this_tile.items():
                            try:
                                # v is in there, adjust multiplier. Otherwise exception is raised
                                index_per_row = [i[0] for i in cost_model_evaluations_per_layer_per_row[k]].index(v)
                                # Note: CostModelEvaluation has no __eq__ implementation and therefore resorts to comparing
                                # based on id, which is already due to run_for_tilesize actually the exact same instance
                                # due to that it caches its outputs
                                cost_model_evaluations_per_layer_per_row[k][index_per_row][1] += 1
                            except ValueError:
                                # v was not in there, add it with multiplier 1
                                cost_model_evaluations_per_layer_per_row[k].append([v, 1])
                    ...
                    if previous_y0 is None:
                        cost_model_evaluations_per_layer_row_beginning.update(cost_model_evaluations_per_layer_per_row)
                    elif previous_y0 == 0:
                        cost_model_evaluations_per_layer_row_internal.update(cost_model_evaluations_per_layer_per_row)
                    else:
                        cost_model_evaluations_per_layer_row_fin.update(cost_model_evaluations_per_layer_per_row)
                ...
                # TODO: caution! just have moved the following into the substack-iteration
                # Done, accumulate and yield results
                for k in cost_model_evaluations_per_layer:
                    total = None
                    total_tcn_stable_stage = None
                    for i, [cme, mul] in enumerate(cost_model_evaluations_per_layer[k]):
                        cme, extra_info = cme
                        if total is None:
                            total = cme * mul
                        else:
                    ...
                    # yield total, total_tcn_stable_stage, cost_model_evaluations_per_layer[k]
                    yield total, (  self.already_computed_cut_ranges + cut_ranges_to_be_computed,
                                    {'cut_before': cut_before, 'cut_after': cut_after},
                                    cost_model_evaluations_per_layer[k],
                                    cost_model_evaluations_per_layer_row_beginning[k],
                                    cost_model_evaluations_per_layer_row_internal[k],
                                    cost_model_evaluations_per_layer_row_fin[k])
                ...
                for layer, cm_list in cost_model_evaluations_per_layer.items():
                    logger.info(("Layer {layer:>" + str(
                        longest_layer_name) + "s} best energy found = {best_energy:20,d}  with latency {latency:30,d}")
                    ...
                    _total_latency += int(sum(cm[0].latency_total2 * mul for cm, mul in cm_list))
                    _total_energy += int(sum(cm[0].energy_total * mul for cm, mul in cm_list))

                logger.info(("In all: best energy found = {best_energy:20,d}  with latency {latency:30,d}")
                            .format(best_energy=_total_energy,
                                    latency=_total_latency))

            except DFInfiniteLoops as e:
                with open(
                        f'{self.result_saving_path}/{self.tilesize_x}_{self.tilesize_y}_{self.horizontal_caching}_{self.vertical_caching}_errors.log',
                        'a+') as f:
                    f.write(f'Discarding this substack because no DF scheduling is found: {cut_before}, {cut_after}\n')
                logger.info(f'Discarding this substack because no DF scheduling is found: {cut_before}, {cut_after}')
                continue

        with open(
                f'{self.result_saving_path}/general_execution_info.log',
                'a+') as f:
            f.write(f'{self.tilesize_x},{self.tilesize_y},{_evaluated},{len(substacks)}\n')
            print(f'Tile-{self.tilesize_x}x{self.tilesize_y} Actually Evaluated Substacks #[{_evaluated}/{len(substacks)}]')