...
class SpatialMappingGeneratorStage(Stage):
    ...
    def generate_user_spatial_mappings(self):
        ...
        unrollings = []
        # to allow partial specification of spatial mapping DSE
        existed_user_mapping: dict = self.layer.user_spatial_mapping if self.layer.user_spatial_mapping else {}
        for oa_dim in oa_dims:
            oa_dim_unrollings = []
            # to allow partial specification of spatial mapping DSE
            if oa_dim.name in existed_user_mapping:
                oa_dim_unrollings.append(existed_user_mapping[oa_dim.name])
            else:
                for (layer_dim, unrolling_size) in oa_dim_unrolling[oa_dim].items():
